...
class WSAL_NP_Plugin {
	...
	public function remove_built_in_content_notif() {
		...
		if ( $this->wsal->GetGlobalOption( 'notification-built-in-6', false ) ) {
			// Remove the built-in notification: Published content is modified.
			$this->wsal->wsalCommon->DeleteGlobalOption( 'wsal-notification-built-in-6' );
		}
		if ( $this->wsal->GetGlobalOption( 'notification-built-in-7', false ) ) {
			// Remove the built-in notification: Content is modified.
			$this->wsal->wsalCommon->DeleteGlobalOption( 'wsal-notification-built-in-7' );
		}
	}
