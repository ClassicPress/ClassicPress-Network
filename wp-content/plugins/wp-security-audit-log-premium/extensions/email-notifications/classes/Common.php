...
class WSAL_NP_Common {
	...
	public function schedule_daily_summary() {
		if ( '0' === $this->wsal->GetGlobalOption( 'disable-daily-summary', '0' ) ) {
	...
	public function send_daily_summary_report( $test = false ) {
		...
		$summary_email = $this->wsal->GetGlobalOption( 'daily-summary-email', get_bloginfo( 'admin_email' ) );
		$result        = false;
		if ( $summary_email && isset( $report->subject ) && isset( $report->body ) ) {
			$result = $this->SendNotificationEmail( $summary_email, $report->subject, $report->body );
		}
		return $result;
	}
	...
	public function GetEmailTemplate( $name, $force_default = false ) {
		...
		$opt_template = $this->GetOptionByName( $opt_name );
		if ( ! empty( $opt_template ) && ! $force_default ) {
			$template = json_decode( json_encode( $opt_template ), true );
		} else {
			$built_in_subject      = ( 'built-in' == $name ) ? __( 'Built-in Notification', 'wp-security-audit-log' ) : '';
			$default_email_subject = __( ' {title} on website {site} triggered', 'wp-security-audit-log' );
			$template['subject']   = $built_in_subject . $default_email_subject;

			$built_in_body       = ( 'built-in' == $name ) ? 'Built-in email notification' : 'Email Notification';
			$default_email_body  = '<p>' . $built_in_body . __( ' <strong>{title}</strong> was triggered. Below are the notification details:', 'wp-security-audit-log' ) . '</p>';
