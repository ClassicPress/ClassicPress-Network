...
class WSAL_User_Management_Views extends WSAL_AbstractView {
	...
	private function query_users_sessions() {
		// Set the names of the transients.
		$user_sessions_transient        = $this->trans_prefix . 'users_sessions';
		$user_sessions_offset_transient = $this->trans_prefix . 'users_sessions_offset';

		// Check for multisite.
		if ( is_multisite() ) {
			$user_sessions_transient        = $this->trans_prefix . 'users_sessions_' . get_current_blog_id();
			$user_sessions_offset_transient = $this->trans_prefix . 'users_sessions_offset_' . get_current_blog_id();
		}

		// Check user sessions transients.
		$get_fn               = $this->_plugin->IsMultisite() ? 'get_site_transient' : 'get_transient'; // Check for multisite.
		$user_sessions        = $get_fn( $user_sessions_transient );
		$user_sessions_offset = $get_fn( $user_sessions_offset_transient );
		...
		if ( false === $user_sessions || empty( $user_sessions ) || false === $user_sessions_offset ) {
			...
			$set_fn( $user_sessions_transient, $this->user_sessions, DAY_IN_SECONDS );
			$set_fn( $user_sessions_offset_transient, $this->sessions_query_offset, DAY_IN_SECONDS );
		} else {
	...
	public function Footer() {
		...
		wp_enqueue_script(
			...
			array( 'jquery' ),
			filemtime( $this->_base_dir . '/js/script.js' ),
			true
		);
		$script_data = array(
	...
	public function SessionAutoRefresh() {
		...
		$session_nonce = isset( $_POST['session_nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['session_nonce'] ) ) : false;
		// @codingStandardsIgnoreEnd

		// Verify nonce.
		if ( empty( $session_nonce ) || ! wp_verify_nonce( $session_nonce, 'wsal-session-auto-refresh' ) ) {
		...
		if ( ! isset( $_REQUEST['blog_id'] ) ) {
			die( 'Blog ID parameter expected.' );
		}
		...
		if ( $old === $new ) {
			...
			$delete_fn( $user_sessions_transient );
			$delete_fn( $user_sessions_offset_transient );
			echo esc_html( $new );
		}
		die;
	}
	...
	protected function Save() {
		...
		if ( isset( $post_array['_wpnonce'] ) && ! wp_verify_nonce( $post_array['_wpnonce'], 'wsal-users-sessions-save' ) ) {
			return new Exception( __( 'Nonce verification failed', 'wp-security-audit-log' ) );
		}
		...
		if ( isset( $post_array['sessions_error_message'] ) ) {
			$error_message = wp_kses( $post_array['sessions_error_message'], $this->allowed_error_tags );
	...
	public function Render() {
		...
		<div class="tablenav-pages">
			<span class="displaying-num">
				...
				printf( esc_html__( '%s items', 'wp-security-audit-log' ), number_format( $total_sessions ) );
				?>
			</span>
			<?php if ( $pages > 1 ) : ?>
				<span class="pagination-links">
					<a class="first-page<?php echo ( 1 === $paged ) ? ' disabled' : null; ?>" title="<?php esc_attr_e( 'Go to the first page', 'wp-security-audit-log' ); ?>" href="<?php echo esc_url( $first_link ); ?>">«</a>
					<a class="prev-page<?php echo ( 1 === $paged ) ? ' disabled' : null; ?>" title="<?php esc_attr_e( 'Go to the previous page', 'wp-security-audit-log' ); ?>" href="<?php echo esc_url( $prev_link ); ?>">‹</a>
					<span class="paging-input">
						<?php echo absint( $paged ); ?> <?php esc_html_e( 'of', 'wp-security-audit-log' ); ?> <span class="total-pages"><?php echo absint( $pages ); ?></span>
					</span>
					<a class="next-page<?php echo ( $pages === $paged ) ? ' disabled' : null; ?>" title="<?php esc_attr_e( 'Go to the next page', 'wp-security-audit-log' ); ?>" href="<?php echo esc_url( $next_link ); ?>">›</a>
					<a class="last-page<?php echo ( $pages === $paged ) ? ' disabled' : null; ?>" title="<?php esc_attr_e( 'Go to the last page', 'wp-security-audit-log' ); ?>" href="<?php echo esc_url( $last_link ); ?>">»</a>
				</span>
			<?php endif; ?>
		</div>
													...
													foreach ( $result_session as $result ) :
														...
														<tr>
															<td class="username column-username" data-colname="Username">
																...
																<br>
																<?php echo esc_html( $this->_plugin->usermanagement->common->GetUserRoles( $user_id, $result['role'], $current_blog_id ) ); ?>
																<br><br>
															...
															<td class="ip column-ip" data-colname="Source IP">
																...
																if ( ! empty( $result['last_alert']['client_ip'] ) ) :
																	...
																	$ip_address      = add_query_arg( $ip_address_args, 'https://whatismyipaddress.com/ip/' . $result['last_alert']['client_ip'] );
																	?>
															...
															<td class="action column-action" data-colname="Action">
																<?php if ( $this->_plugin->settings->CurrentUserCan( 'edit' ) && wp_get_session_token() != $result['token_hash'] ) : ?>
																	<a href="#"
																		...
																		<?php esc_html_e( 'Terminate Session', 'wp-security-audit-log' ); ?>
																	</a>
																<?php endif; ?>
															</td>
														</tr>
				...
				<?php if ( $this->_plugin->settings->CurrentUserCan( 'edit' ) ) : ?>
					<div class="wsal-tab" id="tab-rules">
						<form id="wsal-rules" method="POST" autocomplete="off">
							<input type="hidden" name="page" value="<?php echo isset( $_REQUEST['page'] ) ? wp_unslash( esc_attr( $_REQUEST['page'] ) ) : false; ?>" />
							...
							<table class="form-table">
								<tbody>
											...
											<fieldset>
												...
												<label for="allow-limited">
													...
													<span>
														...
														printf( esc_html__( 'Allow up to %s sessions', 'wp-security-audit-log' ), $allow_limited );
														?>
													</span>
												</label>
												<br/>
									...
									<tr>
										<td></td>
										<td id="wsal_blocked_session_override">
											...
											<h4><?php esc_html_e( 'Allow blocked sessions to override existing sessions', 'wp-security-audit-log' ); ?></h4>
							...
							<table class="form-table">
								<tbody>
									<tr>
										...
										<td>
											<fieldset>
												<?php $error_message = $this->_plugin->usermanagement->common->GetOptionByName( 'user-management-sessions-error-message', __( 'ERROR: Your session was blocked with the <a href="https://en-gb.wordpress.org/plugins/wp-security-audit-log" target="_blank">WP Security Audit Log plugin</a> because there is already another user logged in with the same username. Please contact the site administrator for more information.', 'wp-security-audit-log' ) ); ?>
												<label for="sessions-error-message">
													<textarea rows="5" cols="50" name="sessions_error_message" id="sessions-error-message" style="margin-top: 2px;"><?php echo wp_kses( $error_message, $this->allowed_error_tags ); ?></textarea>
												</label>
											</fieldset>
										</td>
									</tr>
	...
	public function terminate_all_sessions() {
		...
		$get_fn = $this->_plugin->IsMultisite() ? 'get_site_transient' : 'get_transient'; // Check for multisite.
		if ( is_multisite() ) {
			$users_sessions = $get_fn( 'wsal-users_sessions_' . get_current_blog_id() );
		} else {
