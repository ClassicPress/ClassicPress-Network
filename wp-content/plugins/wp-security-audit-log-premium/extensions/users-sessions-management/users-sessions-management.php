...
class WSAL_User_Management_Plugin {
		...
		if ( $multiple_sessions && 'allow-limited' !== $multiple_sessions ) {
			...
			if ( 'without_warning' === $session_override ) {
				...
				if ( empty( $wsal_override_password ) ) {
					...
					if ( empty( $msg ) ) {
						$msg = __( 'ERROR: Your session was blocked with the <a href="https://en-gb.wordpress.org/plugins/wp-security-audit-log" target="_blank">WP Security Audit Log plugin</a> because there is already another user logged in with the same username. Please contact the site administrator for more information.', 'wp-security-audit-log' );
					}
	...
	public function notify_multi_sessions( $user_login, $current_user = null ) {
		...
		if ( $count > 1 ) {
			...
			if ( is_multisite() ) {
				$delete_fn( 'wsal-users_sessions_offset_' . get_current_blog_id() );
			} else {
	...
	public function auto_destroy_sessions() {
		...
		if ( is_multisite() ) {
			$users_activity = $get_fn( 'wsal-users_sessions_' . get_current_blog_id() );
		} else {
