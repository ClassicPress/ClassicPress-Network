...
class WSAL_Rep_Common {
	...
	final public static function GetCurrentBlogInfo() {
		...
		if ( is_multisite() ) {
			$t->blogname = get_blog_option( $blog_id, 'blogname' );
			$t->domain   = $wpdb->get_var( 'SELECT domain FROM ' . $wpdb->blogs . ' WHERE blog_id=' . $blog_id );
		}
		return $t;
	}
	...
	public function SwitchToArchiveDB() {
		if ( $this->IsArchivingEnabled() ) {
			$archive_type       = $this->GetOptionByName( 'archive-type' );
			$archive_user       = $this->GetOptionByName( 'archive-user' );
			$password           = $this->GetOptionByName( 'archive-password' );
			$archive_name       = $this->GetOptionByName( 'archive-name' );
			$archive_hostname   = $this->GetOptionByName( 'archive-hostname' );
			$archive_baseprefix = $this->GetOptionByName( 'archive-base-prefix' );
			$archive_ssl        = $this->GetOptionByName( 'archive-ssl', false );
			$archive_cc         = $this->GetOptionByName( 'archive-client-certificate', false );
			$archive_ssl_ca     = $this->GetOptionByName( 'archive-ssl-ca', false );
			$archive_ssl_cert   = $this->GetOptionByName( 'archive-ssl-cert', false );
			$archive_ssl_key    = $this->GetOptionByName( 'archive-ssl-key', false );
