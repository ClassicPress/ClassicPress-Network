...
class Plugin {
	...
	public function pre_can_user( $verdict, $args ) {
		...
		// Administrators on global.classicpress.net are considered global admins in GlotPress.
	 ...
	 *
	 * Users present as an administrator on global.wordpress.net are treated as a
	...
	public function is_global_administrator( $user_id ) {
		...
		// 115 = global.wordpress.net. Administrators on this site are considered global admins in GlotPress.
	...
	public function get_blog_prefix( $locale_slug ) {
		...
		$result = get_sites( [
			...
			'domain'     => "$subdomain.wordpress.net",
			'path'       => '/',
			'number'     => 1,
		] );
