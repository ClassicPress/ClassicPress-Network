 ...
 * Locale Route Class.
 *
 * Provides the route for translate.classicpress.org/locale/$locale.
 */
class Locale extends GP_Route {
	...
	private function get_project_icon( $project, $sub_project, $size = 128 ) {
		...
		switch ( $project->slug ) {
			case 'wp':
				return '<div class="classicpress-icon"><span class="dashicons dashicons-wordpress-alt"></span></div>';
			case 'meta':
	...
	private function get_locale_contributors( $project, $locale_slug, $set_slug ) {
		...
		// Check if the project has sub-projects, like /wp-plugins/classicpress-importer.
